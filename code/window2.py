...
class Window:
    ...
    def clear_rectangles(self):
        ...
        for rect in self.rectangles:
            rect.remove()  # Remove from Matplotlib plot
        self.rectangles.clear()  # Clear the stored list
        self.fig.canvas.draw()  # Force refresh of the figure

    def draw_rect(self, x, y, width, height, color='red'):
        """
        Draw a red rectangle on the Matplotlib figure, replacing the old one.
        ...
        if self.prev_rect:
            self.prev_rect.remove()

        # Draw a new rectangle
        self.prev_rect = patches.Rectangle(
            (x, y), width, height, linewidth=2, edgecolor=color, facecolor='none'
        )
        self.ax.add_patch(self.prev_rect)
